...
const PIXEL_CHANNELS = 24 * PIXELS_IN_REGION;
...
function findSimilarDomains(data, domains) {
    ...
    for (let regionIndex = 0; regionIndex < REGIONS_IN_IMAGE; regionIndex++) {
        ...
        // let regionObject = formMatrix(originalData, regionIndex, REGION_SIZE);
        ...
        //Ищем похожий домен методом наименьших квадратов
            ...
            // for (let performanceIndex = 0; performanceIndex < performedDomains.length; performanceIndex++) {
                ...
                // for (let i = 0; i < REGION_SIZE; i++) {
                    // for (let j = 0; j < REGION_SIZE; j++) {
                        // distance += (region[i][j][0] - domain[i][j][0]) ** 2;
                        // distance += (region[i][j][1] - domain[i][j][1]) ** 2;
                        // distance += (region[i][j][2] - domain[i][j][2]) ** 2;
                    // }
                // }
